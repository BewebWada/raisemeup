...
try {
    ...
    // 後からfamily_webhook.php側のfollowイベントで確認できた時点で同じ処理を行う
    if ($isFriend) {
        $lineClient = new LineClient(Config::get('LINE_FAMILY_CHANNEL_SECRET'), Config::get('LINE_FAMILY_CHANNEL_ACCESS_TOKEN'));
        FriendConfirmationService::confirmFamily($familyRepo, $lineClient, ['id' => $pending['id'], 'name' => $pending['name'], 'line_user_id' => $profile['userId']]);
    }
