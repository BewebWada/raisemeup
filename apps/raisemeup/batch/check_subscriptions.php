...
// 本人(users.status='pending')・ご家族(family_accounts.line_user_id IS NULL)のどちらか一方でも
// 連携が完了しないままABANDON_TIMEOUT_DAYS日を超えた契約は、一度も使われていないサービスへの課金を
// 防ぐためStripe契約をキャンセルし、再申込みができるようfamily_accounts.emailを解放する
// (UNIQUE制約が永久に再申込みを妨げないように)。ご家族の連携も必須のため、本人だけ連携済みでも対象になる。
...
$stmt = $pdo->query(
    "SELECT u.invite_code AS user_invite_code, u.display_name AS user_display_name,
            ...
            (u.status = 'pending') AS user_pending, (fa.line_user_id IS NULL) AS family_pending
     FROM subscriptions s
     JOIN users u ON u.id = s.user_id
     JOIN family_accounts fa ON fa.id = s.family_account_id
     WHERE s.status = 'trial' AND (u.status = 'pending' OR fa.line_user_id IS NULL)
...
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    $body = "{$row['family_name']}様\n\n"
        . "TAYORIへのお申込みありがとうございます。{$pendingWho}LINE連携がまだ完了していないようです。\n"
        . "ご本人・ご家族様どちらの連携も必須のため、このまま完了しない場合、明日にはお申込みを一旦キャンセルさせていただきます。\n\n"
...
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    if (!empty($row['family_email'])) {
        ...
        $body = "{$row['family_name']}様\n\n"
            . "{$pendingWho}LINE連携が完了しないまま" . ABANDON_TIMEOUT_DAYS . "日が経過したため、"
